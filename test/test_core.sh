...
test_hyphenated_theme_preservation() {
  ...
  rm -f "$tmp_home/.local/state/walh/current_theme"
  ln -s "$REPO_DIR/scripts/catppuccin-mocha.sh" "$tmp_home/.local/state/walh/current_theme"
  output="$(HOME="$tmp_home" bash "$REPO_DIR/profile_helper.sh")"
  ...
  rm -f "$tmp_home/.local/state/walh/current_theme"
  ln -s "$REPO_DIR/scripts/onedark.sh" "$tmp_home/.local/state/walh/current_theme"
  output="$(HOME="$tmp_home" bash "$REPO_DIR/profile_helper.sh")"
...
# ---------------------------------------------------------------------------
# Test 6: XDG Base Directory Theme Restoration
# ---------------------------------------------------------------------------
test_xdg_base_directory_state_file() {
  local tmp_home
  tmp_home="$(mktemp -d)"

  # Case 1: Default ~/.local/state/walh/current_theme
  mkdir -p "$tmp_home/.local/state/walh"
  ln -s "$REPO_DIR/scripts/gruvbox-dark.sh" "$tmp_home/.local/state/walh/current_theme"
  local output
  output="$(HOME="$tmp_home" bash "$REPO_DIR/profile_helper.sh")"
  assert_contains "$output" "export WALH_THEME=gruvbox-dark" "profile_helper detects default XDG state file"
  assert_contains "$output" "$tmp_home/.local/state/walh/current_theme" "profile_helper sources from default XDG state file"

  # Case 2: Custom XDG_STATE_HOME
  local custom_state="$tmp_home/custom_state"
  mkdir -p "$custom_state/walh"
  ln -s "$REPO_DIR/scripts/onedark.sh" "$custom_state/walh/current_theme"
  output="$(HOME="$tmp_home" XDG_STATE_HOME="$custom_state" bash "$REPO_DIR/profile_helper.sh")"
  assert_contains "$output" "export WALH_THEME=onedark" "profile_helper detects custom XDG_STATE_HOME"
  assert_contains "$output" "$custom_state/walh/current_theme" "profile_helper sources from custom XDG_STATE_HOME"

  rm -rf "$tmp_home"
}

# ---------------------------------------------------------------------------
# Test 7: Startup Alias Behavior (Zero Polluting Theme Aliases)
# ---------------------------------------------------------------------------
test_startup_alias_behavior() {
  local tmp_home
  tmp_home="$(mktemp -d)"

  # Default startup: zero theme aliases, walh function loaded via walh.sh
  local output
  output="$(HOME="$tmp_home" bash "$REPO_DIR/profile_helper.sh")"
...
EOF
  ...
  if [ -L "$tmp_home/.local/state/walh/current_theme" ] && [ ! -e "$tmp_home/.walh_theme" ]; then
    echo "PASS: XDG state symlink created at ~/.local/state/walh/current_theme and ~/.walh_theme absent"
  else
    echo "FAIL: expected symlink at ~/.local/state/walh/current_theme and no ~/.walh_theme"
    FAILED=1
  fi
